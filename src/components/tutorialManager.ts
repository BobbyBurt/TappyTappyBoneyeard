...
export default class tutorialManager {
	...
	public static getTutorialText(levelIndex: number): string | null
	{
		...
		[
			// level 1
				`Welcome to life, Bird Tapper! Let's brush up on the basics.\n\nYou can jump with the ${InputManager.getJumpInputName()}. You'll move forward automatically, but you can change direction from walls. While against one, try jumping against it and jumping again to perform a wall jump.`,
			
			// level 2
				`Have you tried using your wings yet? Use the ${InputManager.getJumpInputName()} while midair to flap and gain some more height.\n\nYour tiny wings only have the strength to do this twice, and you'll get more and more pale as you become weaker.`,

			// level 3
				`Soldiers are dangerous, so it's time to attack! Hit the ${InputManager.getPunchInputName()} to punch forward with a burst of speed.\n\nDo this in front of an enemy to take them out with your fist. Punches are also an easy way to launch yourself from a wall.`,
				
			// level 4
				null,

			// level 5
				`Time to dive. Use the ${InputManager.getDiveInputName()} to boost yourself downwards. If you hit an enemy, you'll take them out!\n\nDiving also aids your mobility, helping you control / time your landings.`,

			// level 6
				null,
			
			// level 7
				`When punching forward isn't enough, hit 'em with an uppercut!\n\nUse the ${InputManager.getUppercutInputName()} to launch upward and take out the enemies above you.`,
